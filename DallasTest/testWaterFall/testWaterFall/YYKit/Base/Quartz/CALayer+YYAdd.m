...
+ (CAGradientLayer *)drawLinearGradientInRect:(CGRect)rect cornerRadius:(CGFloat)cornerRadius colors:(NSArray *)colors points:(NSArray *)points
{
    if (colors.count<2||points.count<2||rect.size.width<=0||rect.size.height<=0) {
        return nil;
    }
    
    CAGradientLayer *layer = [CAGradientLayer new];
    layer.colors = @[(__bridge id)(((UIColor *)colors[0]).CGColor), (__bridge id)(((UIColor *)colors[1]).CGColor)];
    layer.startPoint = CGPointMake(((CGPoint)[(NSValue *)points[0] CGPointValue]).x,((CGPoint)[(NSValue *)points[0] CGPointValue]).y);
    layer.endPoint = CGPointMake(((CGPoint)[(NSValue *)points[1] CGPointValue]).x,((CGPoint)[(NSValue *)points[1] CGPointValue]).y);
    layer.frame = rect;
    layer.cornerRadius = cornerRadius;
    return layer;
}

+ (UIImage *)drawLinearGradientImageInSize:(CGSize)size  colors:(NSArray *)colors locations:(NSArray *)locations points:(NSArray *)points
{
    if (colors.count<2||points.count<2||size.width<=0||size.height<=0) {
        return nil;
    }
    ...
    CGFloat location[] = {[(NSNumber *)locations[0] floatValue],[(NSNumber *)locations[1] floatValue]};
    NSArray * cGcolors = @[(__bridge id)(((UIColor *)colors[0]).CGColor), (__bridge id)(((UIColor *)colors[1]).CGColor)];
    CGGradientRef gradient = CGGradientCreateWithColors(colorSpace, (CFArrayRef)cGcolors,location);
    CGColorSpaceRelease(colorSpace);
    
    CGPoint startPoint = CGPointMake(size.width * ((CGPoint)[(NSValue *)points[0] CGPointValue]).x,size.height * ((CGPoint)[(NSValue *)points[0] CGPointValue]).y);
    CGPoint endPoint = CGPointMake(size.width * ((CGPoint)[(NSValue *)points[1] CGPointValue]).x,size.height * ((CGPoint)[(NSValue *)points[1] CGPointValue]).y);
